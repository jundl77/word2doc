...
class TensorBoard(Callback):
    ...
    ```
    # Arguments
        custom_log_func: a functions that returns a dict with custom logs that should be added to
            tensorboard, in the same format as the logs dict. This function takes the same params as on_epoch_end
        ...
        if self.embeddings_freq and self.embeddings_ckpt_path:
            if epoch % self.embeddings_freq == 0:
                self.saver.save(self.sess,
                                self.embeddings_ckpt_path, epoch)
